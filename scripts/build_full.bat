...
if "%PROXY_URL%"=="" (
  if "%PROXY_HOST%"=="" set "PROXY_HOST=192.168.8.101"
...
if "%IMAGE_TAG%"=="" set "IMAGE_TAG=clash2web:rebuid"

echo == docker build full image (with scripts + web) ==
...
echo.
echo [3/4] Build image with Dockerfile.base...
...
echo.
echo [4/4] Verify image...
docker run --rm --entrypoint /bin/sh "%IMAGE_TAG%" -lc "mihomo -v | head -n 1 && ls -lh /usr/local/share/mihomo/geoip.metadb && ls /scripts && ls /web"
if errorlevel 1 (
...
echo.
echo To export: docker save %IMAGE_TAG% -oDesktop\clash2web-rebuid.tar
exit /b 0
