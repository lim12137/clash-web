...
- 补充修正默认分组选择与空态逻辑，避免在 Docker 运行态优先落到 `Proxy/Google` 这类只有子组、没有直接节点卡片的分组，导致用户误判为“节点视图失效”。
...
- 用户反馈表明 Docker 运行态的上一版视图修复不完整，因此本次继续补了前端默认选组与空态策略；当前工作站未连通 Docker Engine，未能直接在本机容器内复验。
